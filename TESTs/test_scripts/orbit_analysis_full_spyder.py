...
# -*- coding: utf-8 -*-
"""
orbit_analysis_full_spyder.py

Full satellite-orbit analysis pipeline for Gadget/Gadget-4 HDF5 snapshots.

This script is designed for Spyder:
  1. edit only the USER SETTINGS block at the bottom;
  2. press F5 / Run file;
  3. inspect the generated pandas objects in the Variable Explorer:
        results
        combined_df
        extrema_all_df
...
What it does
------------
For each simulation label in

    root / LABEL / output / snapshot_*.hdf5

the script:
  - reads gas, dark matter, and star particles;
  - builds an initial satellite ID catalogue;
  - defines initial central stellar IDs from stars_com at the first snapshot;
  - tracks the satellite centre through time using several possible methods;
  - computes orbital radius, Vrad, Vtan, V3d;
  - estimates instantaneous tidal radius in an analytic NFW host;
  - estimates stellar membership / stripping using tidal and optional kinematic cuts;
  - measures stellar/gas/DM masses, sizes, SFR, sSFR, SFE, depletion time;
  - estimates ram pressure from local CGM gas;
  - estimates simple tidal-field proxies;
  - finds pericentres and apocentres;
  - saves CSV/NPZ/JSON outputs.

Recommended centre mode
-----------------------
For your current tests, use:

    center_mode = "inner_ids_shrinking"
    velocity_mode = "inner_ids"

This defines a central stellar population using stars_com in the initial snapshot,
then follows those ParticleIDs through time. This avoids the all-stars COM being
pulled by tidal tails after stripping.

Author: generated/adapted for Abhner's orbit-analysis workflow.
"""
...
# Edit this block and press F5 in Spyder.
...
if __name__ == "__main__":
    if RUN_STRUCTURE_INSPECTION:
        ...
        first_snap = find_snapshots_for_label(cfg, labels_to_check[0])[0]
        inspect_snapshot_structure(first_snap)
