...
# -*- coding: utf-8 -*-
"""
dwarf_ram_analysis.py

Organized analysis module for controlled Gadget4 dwarf-galaxy runs.
...
OUTPUT_COLUMNS = [
    ...
    "rhalf_gas_sf_kpc",
    "rho_cgm_g_cm3", "vrel_km_s", "pram_dyn_cm2", "n_env",
...
def build_cgm_shell_mask(ds: Any, gas: Dict[str, Any], dwarf_center_native: Sequence[float], rin_kpc: float, rout_kpc: float, cgm_temp_min: float, cgm_density_max: Optional[float]) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
...
def analyze_loaded_snapshot(
    ...
    if env is None:
        env = {
            "rho_cgm_g_cm3": np.nan, "vrel_km_s": np.nan, "pram_dyn_cm2": np.nan, "n_env": 0,
...
def analyze_snapshot_directory(
    ...
    for i, snap in enumerate(snapfiles):
        ...
        try:
            rec, new_dwarf, new_host = analyze_snapshot_file(snap, dwarf_guess, cfg, host_guess)
        except Exception as exc:
